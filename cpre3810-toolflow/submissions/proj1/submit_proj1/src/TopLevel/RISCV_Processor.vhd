...
architecture structure of RISCV_Processor is
  ...
  signal s_FtD_Reg      : std_logic_vector(95 downto 0);  -- 96 bit output
  ...
  signal s_DtE_Reg      : std_logic_vector(179 downto 0); -- 177 bit output
  ...
  signal s_EtM_Reg      : std_logic_vector(75 downto 0);  -- 76 bit output
  ...
  signal s_MtWB_Reg     : std_logic_vector(70 downto 0); -- 98 bit output
...
---------------- DECODE STAGE ----------------
  ...
  Fetch_To_Decode_Reg: FetchDecode_Reg
    generic map(N => 96)      -- 96 bit register
    port map(i_CLK  => iCLK,
             i_RST  => iRST,
             i_WE   => not s_Halt,   -- Consider using this port to stall as well in the future
             i_D    =>   s_PC4    -- PC+4 Value     -- [95:64]
                       & s_PC     -- PC Value       -- [63:32]
...
---------------- EXECUTE STAGE ----------------
  ...
  generic map(N => 180)      -- 180 bit register
  port map(i_CLK => iCLK,
           i_RST => iRST,
           i_WE  => not s_Halt,   -- Change this later for stalling (Add onto it (Logical OR(?) ) )
           i_D   =>  s_funct3     -- RawFunct3_fr_Load  -- [179:177]
...
---------------- MEMORY STAGE ----------------
  ...
  Execute_To_Memory_Reg: ExecuteMemory_Reg
    generic map(N => 76)      -- 76 bit register
    port map(i_CLK => iCLK,
             i_RST => iRST,
             i_WE  => not s_Halt,
             i_D   =>   s_DtE_Reg(179 downto 177)   -- Funct3 bits--[75:73]
...
Memory_To_WriteBack_Reg: MemoryWriteback_Reg 
  generic map(N => 71)    -- 71 bit register
  port map(i_CLK => iCLK,
           i_RST => iRST,
           i_WE  => not s_Halt,
           i_D   => s_EtM_Reg(72)             -- RegWrite   -- [70]
